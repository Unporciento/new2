...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  //  KPI PANEL — sincronización con eventos de fuga
  // ═══════════════════════════════════════════════════════════════════════════
  let _kpiFugas = 0;
  let _kpiLitros = 0;

  function _updateKPIs() {
    const fEl = document.getElementById('kpi-fugas');
    const lEl = document.getElementById('kpi-litros-totales');
    const card = document.getElementById('kpi-fugas-card');
    if (fEl) fEl.textContent = _kpiFugas;
    if (lEl) lEl.textContent = _kpiLitros < 1000
      ? `${_kpiLitros.toFixed(0)} L`
      : `${(_kpiLitros / 1000).toFixed(2)} m³`;
    if (card) card.classList.toggle('kpi-card--danger', _kpiFugas > 0);
  }

  window.addEventListener('leak:started', () => {
    _kpiFugas++;
    _updateKPIs();
  });

  // Sincroniza litros cada 2s desde el contador del sidebar
  setInterval(() => {
    const lcVal = document.getElementById('lc-value');
    if (!lcVal) return;
    const txt = lcVal.textContent;
    const parsed = parseFloat(txt.replace(',', '.'));
    if (!isNaN(parsed) && parsed > _kpiLitros) {
      _kpiLitros = txt.includes('m³') ? parsed * 1000 : parsed;
      _updateKPIs();
    }
  }, 2000);
